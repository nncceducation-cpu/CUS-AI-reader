...
CUS AI Reader is a research-use web application for neonatal cranial ultrasound study review. It accepts a single image, an image set, DICOM objects, or cine clips. The app samples frames, performs basic technical quality checks, records clinician-verified imaging features, applies the four-step Canadian consensus GMH-IVH and PVHI rules independently to the left and right hemispheres, classifies serial WMI and cerebellar hemorrhage patterns, assesses PHVD thresholds, and exports an auditable JSON or Markdown record.
...
2. Review sampled frames and technical QC flags.
...
The current contract expects an ONNX output shaped `[batch, labels]`, with one probability per manifest label. Model probabilities remain separate from clinician-verified evidence. The deterministic consensus rules are in `cus_ai/clinical.py`.
...
- Single image, image set, DICOM, and clip sampling: implemented
